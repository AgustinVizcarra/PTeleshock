...
public class PedidosGeneralDao extends BaseDao {
    ...
    public ArrayList<BPedidoGeneral> buscarPedidosGeneral(int inicio, int idCliente, String texto) {
        //msql

        ArrayList<BPedidoGeneral> listaPedidosGeneral = new ArrayList<>();

        String sql = "select un.idusuario,p.idpedido, p.codigodeventa, p.fechapedido, sum(round((d.cantidad*pxf.preciounitario),2)) as 'Precio total c/producto' from usuario un\n" +
                "left join pedido p on (un.idusuario = p.idusuario)\n" +
                "left join estatuspedido e on (p.idestatuspedido = e.idestatuspedido)\n" +
                "left join detallepedido d on (p.idpedido = d.idpedido)\n" +
                "left join productoporfarmacia pxf on ((d.idproducto = pxf.idproducto) and (d.idfarmacia = pxf.idfarmacia))\n" +
                ...
                "left join farmacia f on (pxf.idfarmacia = f.idfarmacia)\n" +
                "left join usuario u on (f.idusuario = u.idusuario)\n" +
                "where un.idusuario = ? and e.idestatuspedido not in (1) and (p.codigodeventa like ?  or date(p.fechapedido) like ?)\n" +
                "group by p.codigodeventa, p.fechapedido order by p.fechapedido desc limit " + inicio + ",5 ;";
    ...
    public int obtenerNumFilasPGBuscador(int idCliente, String texto) {

        int numFilasPG = 0;

        String sql = "select count(*) from (select un.idusuario,p.idpedido, p.codigodeventa, p.fechapedido, sum(round((d.cantidad*pxf.preciounitario),2)) as 'Precio total c/producto' from usuario un\n" +
                "left join pedido p on (un.idusuario = p.idusuario)\n" +
                "left join estatuspedido e on (p.idestatuspedido = e.idestatuspedido)\n" +
                "left join detallepedido d on (p.idpedido = d.idpedido)\n" +
                "left join productoporfarmacia pxf on ((d.idproducto = pxf.idproducto) and (d.idfarmacia = pxf.idfarmacia))\n" +
                ...
                "left join usuario u on (f.idusuario = u.idusuario)\n" +
                "where un.idusuario = ? and e.idestatuspedido not in (1) and (p.codigodeventa like ?  or date(p.fechapedido) like ?) \n" +
                "group by p.codigodeventa, p.fechapedido order by p.fechapedido desc  ) as `pedidos`;";
