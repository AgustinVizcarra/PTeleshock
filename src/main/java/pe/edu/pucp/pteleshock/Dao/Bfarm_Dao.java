...
public class Bfarm_Dao extends BaseDao {
    ...
    public ArrayList<Bbloqueadas> listar_bloqueadas(String pag) {
        ...
        try (Connection conn = this.getConnection();
            ...
            e.printStackTrace();
            System.out.println("Error al listar historial de FBloqueadas con paginacion");
        }
        return bbloqueadas;
    }

    public int cantidadListaBloqueadas() {
        ArrayList<Bbloqueadas> bbloqueadas = new ArrayList<>();
        try (Connection conn = this.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("select a.nombre 'Administrador',farm.nombre 'Farmacia' ,historialadmin.razon 'Razon/Motivo' ,historialadmin.fechahora 'Fecha' from historialadmin \n" +
                     "inner join farmacia f on (historialadmin.idfarmacia = f.idfarmacia)\n" +
            ...
            e.printStackTrace();
            System.out.println("Error al listar historial de FBloqueadas");
        }
        return bbloqueadas.size();
    }
